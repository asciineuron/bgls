...
class BglsSimulator(cirq.sim.SimulatesSamples):
    ...
    def _run(
        ...
        # second or option in case empty
        ...
        for repetition in range(repetitions):
            ...
            for op in resolved_circuit.all_operations():
                ...
                if isinstance(op.gate, cirq.MeasurementGate):
                    ...
                    records[meas_key][repetition, 0, :] = np.fromstring(
                        bitstring,
                        "u1",
                    ) - ord("0")
                    break
                ...
                candidates = list(
                    itertools.product(
                        *[
                            ["0", "1"] if i in op_support  # Update bits on
                            # operation
                            # support.
                ...
                state = self._ryan_apply(
                    op, state, qubits
                )  # TODO: Make this an input.
                probs = [
                    abs(state[int("".join(bits), 2)]) ** 2
                ...
                bitstring = "".join(
                    candidates[
                        rng.choice(
                            range(len(candidates)), p=probs / sum(probs)
                        )
                    ]
                )
